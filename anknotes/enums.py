...
class AutoNumber(Enum):
		...
		def myParent(self):
			...
			if callable(my_parent_getattr): return my_parent_getattr()			
			else: return my_parent_getattr			
		...
		def myParentGetAttr(self, attrName):
			...
			if callable(my_parent_attr_getattr): return my_parent_attr_getattr()			
			else: return my_parent_attr_getattr	
		...
		def offset(self, level=None):
			if level and self._offset_lambda_:
				...
				if isinstance(o, int):
					o = o, o
				return o				
			if not hasattr(self, 'offset_start_static'): o = self.id()
			else: o = self.offset_start_static()
			return o, o
			...
			if not isinstance(o, int):
				try:
					...
					if range_tuple[0] == range_tuple[1]:
						...
						print "Unexpected range tuple for titlePart offset for offset_as_scalar. Offset: %s " % str(range_tuple)
				except:
					print "Fail - "					
			return o    		
