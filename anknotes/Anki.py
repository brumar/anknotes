...
class Anki:        
    ...
    def get_templates(self):                
        field_names = {"Title": FIELDS.TITLE, "Content": FIELDS.CONTENT, "Extra": FIELDS.EXTRA, "See Also": FIELDS.SEE_ALSO, "TOC": FIELDS.TOC, "Outline": FIELDS.OUTLINE, "Evernote GUID Prefix": FIELDS.EVERNOTE_GUID_PREFIX, "Evernote GUID": FIELDS.EVERNOTE_GUID}
    ...
    def get_evernote_guids_from_anki_note_ids(self, ids, process_usns=True):
        ...
        for a_id in ids:
            ...
            if FIELDS.UPDATE_SEQUENCE_NUM in fields: self.usns[evernote_guid] = fields[FIELDS.UPDATE_SEQUENCE_NUM]    
    ...
    def process_see_also_content(self, anki_note_ids):
        ...
        for a_id in anki_note_ids:
            ...
            if not fields[FIELDS.SEE_ALSO]:
                anki_note_prototype = AnkiNotePrototype(self, fields, note.tags, note, count=count, count_update=count_update, max_count=max_count)   
                if anki_note_prototype.fields[FIELDS.SEE_ALSO]:                
                    log("Detected see also contents for Note '%s': %s" % (get_evernote_guid_from_anki_fields(fields), fields[FIELDS.TITLE]))
    ...
    def insert_toc_into_see_also(self):
        ...
        results = db.all("SELECT s.source_evernote_guid, s.target_evernote_guid, n.title FROM %s as s, %s as n WHERE n.guid = s.target_evernote_guid AND s.source_evernote_guid != s.target_evernote_guid AND s.from_toc == 1 ORDER BY s.source_evernote_guid ASC, n.title ASC" % (TABLES.SEE_ALSO, TABLES.EVERNOTE.NOTES))                
        grouped_results = {}
        # log('           INSERT TOCS INTO ANKI NOTES ', 'dump-insert_toc', timestamp=False, clear=True)
        # log('------------------------------------------------', 'dump-insert_toc', timestamp=False)        
        log('           <h1>INSERT TOCS INTO ANKI NOTES</h1> <HR><BR><BR>', 'see_also', timestamp=False, clear=True, extension='htm')
        toc_titles = {}
        for row in results:
            key = row[0]
            value = row[1]
        ...
        for source_guid, toc_guids in grouped_results.items():            
            ...
            if not ankiNote: 
                ...
                log('<h3>%s</h3><BR>' % generate_evernote_span(fields[FIELDS.TITLE], 'Links', 'TOC') + see_also_html + u'<HR>', 'see_also', timestamp=False, extension='htm')
                fields[FIELDS.SEE_ALSO] = see_also_html.replace('evernote:///', 'evernote://')                
                anki_note_prototype = AnkiNotePrototype(self, fields, ankiNote.tags, ankiNote, count=count, count_update=count_update, max_count=max_count)                             
    ...
    def extract_links_from_toc(self):
        ...
        for toc_evernote_guid, fields in toc_evernote_guids.items():
            for match in find_evernote_links(fields[FIELDS.CONTENT]): 
                ...
                query = """INSERT INTO `%s`(`source_evernote_guid`, `number`, `uid`, `shard`, `target_evernote_guid`, `html`, `title`, `from_toc`, `is_toc`) SELECT '%s', %d, %d, '%s', '%s', '%s', '%s', 1, 1 FROM `%s`  WHERE NOT EXISTS (SELECT * FROM `%s` WHERE `source_evernote_guid`='%s' AND `target_evernote_guid`='%s') LIMIT 1 """ % (TABLES.SEE_ALSO, target_evernote_guid, link_number, uid, shard,  toc_evernote_guid, toc_link_html.replace(u'\'', u'\'\''), toc_link_title.replace(u'\'', u'\'\''), TABLES.SEE_ALSO, TABLES.SEE_ALSO, target_evernote_guid, toc_evernote_guid)
    ...
    def insert_toc_and_outline_contents_into_notes(self):    
        linked_notes_fields = {}
        for source_evernote_guid in ankDB().list("select DISTINCT source_evernote_guid from %s WHERE is_toc = 1 ORDER BY source_evernote_guid ASC" % TABLES.SEE_ALSO):
            ...
            toc_count = 0
            outline_count = 0
            for target_evernote_guid, is_toc, is_outline in ankDB().execute("select target_evernote_guid, is_toc, is_outline from %s WHERE source_evernote_guid = '%s' AND (is_toc = 1 OR is_outline = 1) ORDER BY number ASC" % (TABLES.SEE_ALSO, source_evernote_guid)):    
                if target_evernote_guid in linked_notes_fields:
                    ...
                    if linked_note_contents:
                        linked_notes_fields[target_evernote_guid] = {FIELDS.TITLE: linked_note_title, FIELDS.CONTENT: linked_note_contents}
                if linked_note_contents: 
