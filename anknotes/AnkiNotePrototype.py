...
class AnkiNotePrototype:
	...
	def regex_occlude_match(self, match):
		...
		return r'&lt;&lt;' + match.group('PrefixKeep') + ' <div class="occluded">' + match.group('OccludedText') + '</div>&gt;&gt;'
	...
	def process_note_see_also(self):
		...
		for enLink in find_evernote_links(self.Fields[FIELDS.SEE_ALSO]):
			if not check_evernote_guid_is_valid(enLink.Guid): self.Fields[FIELDS.SEE_ALSO] = remove_evernote_link(enLink, self.Fields[FIELDS.SEE_ALSO]); continue 
			link_num += 1
	...
	def process_note_content(self):
		...
		def step_1_modify_evernote_links():
			...
			if self.light_processing: self.Fields[FIELDS.CONTENT] = self.Fields[FIELDS.CONTENT].replace("evernote://", "evernote:///")
		...
		def step_2_modify_image_links():
			...
			self.Fields[FIELDS.CONTENT] = re.sub(r'<a href="%s"[^>]*>(?P<Title>.+?)</a>' % (dropbox_image_url_html_link_regex + dropbox_image_url_suffix),
			...
			try:
				...
				self.Fields[FIELDS.CONTENT] = re.sub(dropbox_image_url_regex, r'\g<URLPrefix>' + dropbox_image_src_subst % "From Plain-Text Link" + r'\g<URLSuffix>', self.Fields[FIELDS.CONTENT])
			except:
		...
		def step_3_occlude_text():
			...
			self.Fields[FIELDS.CONTENT] = re.sub("(?s)(?P<Prefix>&lt;|<) ?(?P=Prefix) ?(?P<PrefixKeep>(?:</div>)?)(?P<OccludedText>.+?)(?P<Suffix>&gt;|>) ?(?P=Suffix) ?", self.regex_occlude_match, self.Fields[FIELDS.CONTENT])
