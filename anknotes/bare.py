...
def main_bare():
    ...
    def print_results():        
        ...
        log.plain(diff, 'SeeAlsoDiff\\Diff\\' + enNote.FullTitle, extension='htm',  clear=True)
        log.plain(n.old.see_also.updated, 'SeeAlsoDiff\\Original\\' + enNote.FullTitle, extension='htm',  clear=True)
        log.plain(n.new.see_also.updated, 'SeeAlsoDiff\\New\\' + enNote.FullTitle, extension='htm',  clear=True)
        log.plain(diff + '\r\n')
    ...
    def process_note():
        ...
        if not n.old.content.regex_original.successful_match:
            ...
            log.plain(enNote.Guid + '\r\n' + ', '.join(enNote.TagNames) + '\r\n' + n.old.content.original, 'SeeAlsoMatchFail\\' + enNote.FullTitle, extension='htm', clear=True)
            ...
            n.new.see_also.updated = n.new.content.original
            n.old.see_also.updated = n.old.content.original
        else:
    ...
    for target_evernote_guid in results:
        ...
        n = notes()
        if tmr.step():
            print "Note #%4s: %4s: %s" % (str(tmr.count), tmr.progress,
                                          enNote.FullTitle if enNote.Status.IsSuccess else '(%s)' % target_evernote_guid)
        ...
        for tag in [EVERNOTE.TAG.TOC, EVERNOTE.TAG.OUTLINE]:
            ...
            flds = ankDB().scalar("SELECT flds FROM notes WHERE flds LIKE '%%%s%s%%'" % (
            FIELDS.EVERNOTE_GUID_PREFIX, target_evernote_guid))
            n.new.see_also = notes.version.pstrings(flds.split("\x1f")[FIELDS.SEE_ALSO_FIELDS_ORD])
            process_note()
            if n.old.see_also.updated == n.new.see_also.updated: continue 
