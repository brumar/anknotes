...
class AnkiNotePrototype:
    ...
    def process_note_see_also(self):
        ...
        for match in find_evernote_links(self.fields[FIELDS.SEE_ALSO]):
            ...
            ankDB().execute("INSERT INTO %s (source_evernote_guid, number, uid, shard, target_evernote_guid, html, title, from_toc, is_toc, is_outline) VALUES('%s', %d, %d, '%s', '%s', '%s', '%s', 0, %d, %d)" % (TABLES.SEE_ALSO, self.evernote_guid,link_num, int(match.group('uid')), match.group('shard'), match.group('guid'), match.group('Title'), title_text, is_toc, is_outline))
    ...
    def process_note_content(self):    
        ...
        remove_style_attrs = '-webkit-text-size-adjust: auto|-webkit-text-stroke-width: 0px|background-color: rgb(255, 255, 255)|color: rgb(0, 0, 0)|font-family: Tahoma|font-size: medium;|font-style: normal|font-variant: normal|font-weight: normal|letter-spacing: normal|orphans: 2|text-align: -webkit-auto|text-indent: 0px|text-transform: none|white-space: normal|widows: 2|word-spacing: 0px'.replace('(', '\\(').replace(')', '\\)')
    ...
    def log_update(self, content=''):
        if not self.logged:
            ...
            log(log_title, 'AddUpdateNote', timestamp=(content is ''), clear=((self.count==1 or self.count==100) and not self.logged))
            self.logged = True
            ...
            for field_to_update in fields_to_update:                
                if field_to_update in fld.get('name') and field_to_update in self.fields:
                    ...
                    try:
                        ...
                        log_error("ERROR: UPDATE_NOTE: Note '%s': %s: Unable to set self.note.fields for field '%s'. Ord: %s. Note fields count: %d" % (self.evernote_guid, self.fields[FIELDS.TITLE], field_to_update, str(fld.get('ord')), len(self.note.fields)) )
                        raise        
        if len(field_updates) == 2:
            if FIELDS.SEE_ALSO in fields_updated and FIELDS.CONTENT in fields_updated:
                ...
                if fc_test: 
                    ...
                    log_dump([fc_test1, fc_test2, self.unprocessed_content , '-' + fields_updated[FIELDS.CONTENT]], 'AddUpdateNoteTest')                
    ...
    def add_note(self):
        ...
        if self.note is not None:
            ...
            try:
                ...
                log_error("Unable to collection.addNote for Note %s:    %s" % (self.fields[FIELDS.EVERNOTE_GUID].replace(FIELDS.EVERNOTE_GUID_PREFIX, ''), db_title))
