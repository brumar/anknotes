...
class ank_DB(object):
    ...
    def commit(self):
        ...
        if self.echo:
            print
            "commit %0.3fms" % ((time.time() - t) * 1000)
    ...
    def InitSeeAlso(self, forceRebuild=False):
        ...
        if forceRebuild:
            ...
            if_exists = ""
        self.execute(
            """CREATE TABLE %s `%s` ( `id` INTEGER, `source_evernote_guid` TEXT NOT NULL, `number` INTEGER NOT NULL DEFAULT 100, `uid` INTEGER NOT NULL DEFAULT -1, `shard` TEXT NOT NULL DEFAULT -1, `target_evernote_guid` TEXT NOT NULL, `html` TEXT NOT NULL, `title` TEXT NOT NULL, `from_toc` INTEGER DEFAULT 0, `is_toc` INTEGER DEFAULT 0, `is_outline` INTEGER DEFAULT 0, PRIMARY KEY(id) );""" % (
