...
def setup_evernote(self):
    ...
    evernote_query_tags.setText(
        mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.TAGS, SETTINGS.EVERNOTE.QUERY.TAGS_DEFAULT_VALUE))
    ...
    evernote_query_use_tags.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_TAGS, True))
    ...
    evernote_query_excluded_tags.setText(
        mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.EXCLUDED_TAGS, ''))
    ...
    evernote_query_search_terms.setText(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.SEARCH_TERMS, ""))
    ...
    evernote_query_use_search_terms.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_SEARCH_TERMS, False))
    ...
    evernote_query_notebook.setText(
        mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.NOTEBOOK, SETTINGS.EVERNOTE.QUERY.NOTEBOOK_DEFAULT_VALUE))
    ...
    evernote_query_use_notebook.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_NOTEBOOK, False))
    ...
    evernote_query_note_title.setText(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.NOTE_TITLE, ""))
    ...
    evernote_query_use_note_title.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_NOTE_TITLE, False))
    ...
    evernote_query_last_updated_type.setCurrentIndex(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_TYPE,
    ...
    evernote_query_use_last_updated.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_LAST_UPDATED, False))
    ...
    evernote_pagination_current_page_spinner.setValue(mw.col.conf.get(SETTINGS.EVERNOTE.PAGINATION_CURRENT_PAGE, 1))
    ...
    evernote_pagination_auto_paging.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.AUTO_PAGING, True))
    ...
    update_existing_notes.addItems(["Ignore Existing Notes", "Update In-Place",
                                    "Delete and Re-Add"])
    update_existing_notes.setCurrentIndex(mw.col.conf.get(SETTINGS.ANKI.UPDATE_EXISTING_NOTES,
...
def update_default_anki_deck(text):
    mw.col.conf[SETTINGS.ANKI.DECKS.BASE] = text
    update_anki_deck_visibilities()
...
def update_anki_deck_evernote_notebook_integration():
    if default_anki_deck.text():
        mw.col.conf[
            SETTINGS.ANKI.DECKS.EVERNOTE_NOTEBOOK_INTEGRATION] = anki_deck_evernote_notebook_integration.isChecked()
...
def update_evernote_query_use_tags():
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.USE_TAGS] = evernote_query_use_tags.isChecked()
    update_evernote_query_visibilities()
...
def update_evernote_query_use_excluded_tags():
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.USE_EXCLUDED_TAGS] = evernote_query_use_excluded_tags.isChecked()
    update_evernote_query_visibilities()
...
def update_evernote_query_use_notebook():
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.USE_NOTEBOOK] = evernote_query_use_notebook.isChecked()
    update_evernote_query_visibilities()
...
def update_evernote_query_use_note_title():
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.USE_NOTE_TITLE] = evernote_query_use_note_title.isChecked()
    update_evernote_query_visibilities()


def update_evernote_query_use_last_updated():
    update_evernote_query_visibilities()
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.USE_LAST_UPDATED] = evernote_query_use_last_updated.isChecked()
...
def update_evernote_query_use_search_terms():
    update_evernote_query_visibilities()
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.USE_SEARCH_TERMS] = evernote_query_use_search_terms.isChecked()


def update_evernote_query_any():
    update_evernote_query_visibilities()
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.ANY] = evernote_query_any.isChecked()
...
def update_delete_evernote_query_tags():
    mw.col.conf[SETTINGS.ANKI.TAGS.DELETE_EVERNOTE_QUERY_TAGS] = delete_evernote_query_tags.isChecked()
...
def update_evernote_pagination_current_page_spinner(value):
    ...
    mw.col.conf[SETTINGS.EVERNOTE.PAGINATION_CURRENT_PAGE] = value


def update_update_existing_notes(index):
    mw.col.conf[SETTINGS.ANKI.UPDATE_EXISTING_NOTES] = index
...
def evernote_query_text_changed():
    ...
    if not search_terms:
        ...
        evernote_query_use_search_terms.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_SEARCH_TERMS, True))
    ...
    if not note_title:
        ...
        evernote_query_use_note_title.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_NOTE_TITLE, True))
    ...
    if not notebook:
        ...
        evernote_query_use_notebook.setChecked(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_NOTEBOOK, True))
    ...
    if not tags and not all_inactive:
        ...
        if all_inactive and not tags:
            evernote_query_tags.setText(SETTINGS.EVERNOTE.QUERY.TAGS_DEFAULT_VALUE)
...
def update_evernote_query_last_updated_type(index):
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_TYPE] = index
    evernote_query_last_updated_value_set_visibilities()


def evernote_query_last_updated_value_get_current_value():
    index = mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_TYPE, 0)
    ...
    absolute_time_str = mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_TIME,
...
def evernote_query_last_updated_value_set_visibilities():
    ...
    absolute_date = QDate().fromString(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_DATE,
    ...
    if index < EvernoteQueryLocationType.AbsoluteDate:
        ...
        evernote_query_last_updated_value_relative_spinner.setValue(
            int(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_RELATIVE, 0)))
        ...
        if index == EvernoteQueryLocationType.AbsoluteDate:
            ...
            absolute_time = QTime().fromString(mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_TIME,
...
def update_evernote_query_last_updated_value_relative_spinner(value):
    ...
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_RELATIVE] = value


def update_evernote_query_last_updated_value_absolute_date(date):
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_DATE] = date.toString('yyyy MM dd')


def update_evernote_query_last_updated_value_absolute_datetime(dt):
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_DATE] = dt.toString('yyyy MM dd')
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_TIME] = dt.toString('HH mm ss')


def update_evernote_query_last_updated_value_absolute_time(time_value):
    mw.col.conf[SETTINGS.EVERNOTE.QUERY.LAST_UPDATED_VALUE_ABSOLUTE_TIME] = time_value.toString('HH mm ss')
...
def generate_evernote_query():
    ...
    if mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.ANY, True):
        query += "any: "
    ...
    if mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_TAGS, True):
        tags = mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.TAGS, SETTINGS.EVERNOTE.QUERY.TAGS_DEFAULT_VALUE).replace(',',
                                                                                                                 ' ').split()
        for tag in tags:
            tag = tag.strip()
            if ' ' in tag: tag = '"%s"' % tag
    ...
    if mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.USE_SEARCH_TERMS, False):
        query += mw.col.conf.get(SETTINGS.EVERNOTE.QUERY.SEARCH_TERMS, "")
    return query
