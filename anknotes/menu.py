	...
	def create_subnote(guid):
		def process_lists(lst, names=None, levels=None, under_ul=False):	
			def add_note(contents, names, levels):
				l.go("NOTE:".ljust(16) + '%-6s %-20s %s' % ('.'.join(map(str, levels)) + ':', ': '.join(names), contents), 'notes', crosspost=('.'.join(map(str, levels)) + ' - ' + '-'.join(names))) 
			...
			def process_list_item(contents, under_ul=False):
				...
				for li in contents:
					...
					if not under_ul and (li.name == "ol" or (li.name == "ul" and len(li.contents) > 2)): sublist = li; break 
			...
			for lst_items in lst:
				if isinstance(lst_items, Tag):								
					...
					if len(lst_items.contents) is 0:
						l.go('NO TOP TEXT:'.ljust(16) + '%s%s: %s' % ('\t' * level, '.'.join(map(str, levels)), full_text), crosspost=['notoptext'])
						top_text = "N/A"
					else: top_text = unicode(str(lst_items.contents[0]), 'utf-8') #strip_tags(str(lst_items.contents[0])).strip()					
					if lst_items.name == 'ol' or lst_items.name == 'ul':
						...
						l.go((tag_names[lst_items.name] + ':').ljust(16) + '[%d] %s: <%s>' % (len(levels), '.'.join(map(str, levels)), ''))
						...
						if sublist: 
							...
							l.go('LIST ITEM:      %s%s: %s' % ('\t' * level, '.'.join(map(str, levels)), list_items_full_txt))
					...
					l.go('STRING:'.ljust(16) + '%s%s: %s' % ('\t' * level, '.'.join(map(str, levels)), this_name), crosspost='strings')
				else: 
...
def lxml_test():
	guids = ankDB().list("SELECT guid FROM %s WHERE tagNames LIKE '%%,%s,%%' ORDER BY title ASC " % (TABLES.EVERNOTE.NOTES, TAGS.OUTLINE))
...
Once the command line tool is done running, you will get a summary of the results, and will be prompted to delete Anki Orphan Notes or download Missing Evernote Notes""".replace('\n', '\n<br />') % FILES.USER.TABLE_OF_CONTENTS_ENEX,
	...
	if anknotes_dels_count > 0:
		code = getText("Please enter code 'ANKNOTES_DEL_%d' to delete your orphan Anknotes DB note(s)" % anknotes_dels_count)[0]
...
def validate_pending_notes(showAlerts=True, uploadAfterValidation=True, callback=None, unloadedCollection=False, reload_delay=10):
	if not unloadedCollection: return unload_collection(lambda *xargs, **xkwargs: validate_pending_notes(showAlerts, uploadAfterValidation, callback(*xargs, **xkwargs), True))
...
def modify_collection(collection_operation, action_str='modifying collection', callback=None, callback_failure=False, callback_delay=0, delay=30, attempt=1, max_attempts=5, **kwargs):
	passed=False
	retry = ("Will try again in %ds" % delay + ' (Attempt #%d)' % attempt if attempt > 0 else '') if attempt <= max_attempts else "Max attempts of %d exceeded... Aborting operation" % max_attempts
	try: 
		...
		if e.message.replace(".", "") == 'database is locked': friendly_message = 'sqlite database is locked'
			...
			if e.message.replace('.', '') == 'database is locked': log_error('**locked', crosspost='automation', crosspost_to_default=False)
			import traceback 
			type = str(e.__class__); type = type[type.find("'")+1:type.rfind("'")]
			friendly_message = ('Unhandled Error' if type == 'Exception' else type) + ':\n Full Error: ' + ' '.join(str(e).split()) + '\n Message: "%s"' % e.message +  '\n Trace: ' + traceback.format_exc() + '\n' 
		log_error("   > Modify Collection: Error %s: %s. %s" % (action_str, retry, friendly_message), time_out=10000, do_show_tooltip=True, crosspost='automation', crosspost_to_default=False)
	if not passed: return (False if callback_failure is False else callback(None, **kwargs)) if attempt > max_attempts else mw.progress.timer(delay*1000, lambda: modify_collection(collection_operation, action_str, callback, callback_failure, callback_delay, delay, attempt+1, **kwargs), False)	
	if not callback: log("   > Modify Collection: Completed %s" % action_str, 'automation'); return 
	log("   > Modify Collection: Completed %s" % action_str + ': %s Initiated' % ('%ds Callback Timer' % callback_delay if callback_delay > 0 else 'Callback'), 'automation')
	if not callback: return #return_val
	if callback_delay > 0: mw.progress.timer(callback_delay*1000, lambda: callback(return_val, **kwargs), False); return #return return_val 
...
def reload_collection(callback=None, reopen_delay=0, callback_delay=30, *args, **kwargs):	
	if not mw.col is None: 
		try: 
			...
			result = cur.fetchone()
			log(" > Reload Collection: Not needed: ankDB exists and cursor created: %s" % (str_safe(result[0])), 'automation')
			if callback: callback(True)		
			return True 
		except (sqlite.ProgrammingError, Exception), e:
			if e.message == "Cannot operate on a closed database":			
				# mw.loadCollection()
				log(" > Reloading Collection Check: DB is Closed. Proceed with reload. Col: " + str(mw.col), 'automation')
			else:
				import traceback 
				log(" > Reloading Collection Check Failed : " + str(e) + '\n   - Trace: ' + traceback.format_exc(), 'automation')
	log(" > Initiating Reload: %sInitiated: %s" % ('%ds Timer ' % reopen_delay if reopen_delay > 0 else '', str(mw.col)), 'automation')
	...
	if reopen_delay > 0: return mw.progress.timer(reopen_delay*1000, lambda *xargs, **xkwargs: modify_collection(do_load_collection, 'reload collection', lambda *xargs, **xkwargs: callback(*args, **kwargs), callback_delay=callback_delay, *args, **kwargs), False)
	modify_collection(do_load_collection, 'Reloading Collection', callback, callback_delay=callback_delay, *args, **kwargs)
...
def see_also(steps=None, showAlerts=None, validationComplete=False, controller=None):
	...
	if 3 in steps:
		if validationComplete:
			log(" > See Also: Step 3B: Validate and Upload Auto TOC Notes: Upload Validated Notes", crosspost='automation')
	...
	if 7 in steps:
		if validationComplete:
			log(" > See Also: Step 7B: Validate and Upload Modified Evernote Notes: Upload Validated Notes", crosspost='automation')
			upload_validated_notes(multipleSteps)
		else: steps = [-7]
	if 8 in steps:
	...
	if do_validation>0:
		log(" > See Also: Step %dA: Validate and Upload %s Notes: Validate Notes" % (do_validation, {3: 'Auto TOC', 7: 'Modified Evernote'}[do_validation]), crosspost='automation')
