...
class DictCaseInsensitive2(Dict):
	...
	def __init__(self, *args, **kwargs):
		...
		parent_lbl = self.__process_kwarg__(kwargs, 'parent_label', '')
		print "lbl: %s\nkwargs: %s" % (lbl, str(kwargs))
		if not isinstance(lbl, unicode) and not isinstance(lbl, str): raise TypeError("Cannot create DictCaseInsensitive label from non-string type: " + str(lbl))
		if not isinstance(parent_lbl, unicode) and not isinstance(parent_lbl, str): raise TypeError("Cannot create DictCaseInsensitive parent label from non-string type: " + str(parent_lbl))
		self.__label__ = lbl
