...
class Controller:
    ...
    def create_auto_toc(self):
        def check_old_values():
            ...
            log(generate_diff(old_content, noteBodyUnencoded), 'AutoTOC-Create\\Update\\Diffs\\' + rootTitle,
                clear=True)
            return evernote_guid, contents.replace('/guid-pending/', '/%s/' % evernote_guid).replace('/guid-pending/',
                                                                                                     '/%s/' % evernote_guid)
