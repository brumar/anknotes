...
class Controller:
    ...
    def proceed(self, auto_paging=False):
        ...
        if self.updateExistingNotes is UpdateExistingNotes.IgnoreExistingNotes:
            ...
            diff = n2 - n2_actual
            tooltip = "%d new note(s) have been imported and %d existing note(s) have been updated %s." % (
                n, n2, update_str)
            if diff > 0:
                ...
                tooltip += "<BR>There is a discrepancy of %d note(s) that were not imported into Anki%s." % (
                    n3, reason_str)
