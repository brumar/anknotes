...
TABLE_EVERNOTE_NOTES = "anknotes_evernote_notes"
...
class Anki:
    ...
    def add_evernote_cards(self, evernote_cards, deck, update=False):
        ...
        for card in evernote_cards:
            ...
            if EVERNOTE_TAG_TOC in card.tags:
                ...
                deck += "::" + self.notebook_data[card.notebookGuid].name
                deck = deck.replace(": ", "::")                
    ...
    def create_evernote_tables(self):
        ...
        mw.col.db.execute("""CREATE TABLE `%s` ( `guid` TEXT NOT NULL UNIQUE, `name` TEXT NOT NULL, `updateSequenceNum` INTEGER NOT NULL, `serviceUpdated` INTEGER NOT NULL, `stack` TEXT, PRIMARY KEY(guid) );""" % TABLE_EVERNOTE_NOTEBOOKS)
...
LIMIT 1 """ % (TABLE_SEE_ALSO, link_guid, link_number, uid, shard,  toc_guid, toc_link_html, toc_link_title, TABLE_SEE_ALSO, TABLE_SEE_ALSO, link_guid, toc_guid)
                with open(PATH + '\query.sql', 'w') as filesProgress:
                    print>>filesProgress, query   
...
class Evernote:
    ...
    def check_notebooks_up_to_date(self, guid_set):        
        ...
        for guid in guid_set:
            ...
            if not notebook_guid in notebook_guids and not self.notebook_data[notebook_guid]: 
                notebook_name, notebook_stack = mw.col.db.scalar("SELECT notebook_name, notebook_stack FROM %s WHERE guid = '%s'" % (TABLE_EVERNOTE_NOTEBOOKS, notebook_guid)).fetchone()
    ...
    def process_notebook_guids(self, guid_set):
        try:
            ...
            if e.errorCode == EDAMErrorCode.RATE_LIMIT_REACHED:
                ...
                showInfo("Error: Rate limit has been reached while trying to update Evernote notebooks.\r\n"
    ...
    def check_tags_up_to_date(self, guid_set):        
        ...
        for guid in guid_set:
            ...
            for tag_guid in note_metadata.tagGuids:
                if not tag_guid in tag_guids and not self.tag_data[tag_guid] : 
    ...
    def process_tag_guids(self, guid_set):
        try:
            ...
            if e.errorCode == EDAMErrorCode.RATE_LIMIT_REACHED:
                m, s = divmod(e.rateLimitDuration, 60)
                showInfo("Error: Rate limit has been reached while trying to update Evernote tags.\r\n"
                         "Please retry your request in {} min".format("%d:%02d" % (m, s)))
                return None
            raise
        data = []
...
class Controller:
    ...
    def proceed(self): 
        ...
        for evernote_guid in notes_to_update:
            ...
            if current_usn is self.evernote.metadata[evernote_guid].updateSequenceNum:
...
action = aqt.qt.QAction("Process Evernote TOC", aqt.mw)
