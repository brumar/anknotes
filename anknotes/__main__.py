...
def _findAnknotes((val, args)):
	...
	if val == 'hierarchical' or val == 'hierarchical_alt':
		 if val not in _findAnknotes.note_ids or not ANKNOTES.CACHE_SEARCHES:
			...
			log("  > %s Search Complete: ".ljust(25) % val.upper().replace('_', ' ') + "%-5s --> %3d results" % (tmr.str_long, len(_findAnknotes.note_ids[val])), tmr.label)
	...
	if not hasattr(_findAnknotes, 'queries'):
		_findAnknotes.queries={
			...
			'child_alt': "n.sfld LIKE '%%:%%' AND UPPER(SUBSTR(n.sfld, 0, INSTR(n.sfld, ':'))) IN (SELECT UPPER(title) FROM %s WHERE title NOT LIKE '%%:%%' AND tagNames LIKE '%%,%s,%%') " % (TABLES.EVERNOTE.NOTES, TAGS.TOC),
			'orphan_alt': "n.sfld LIKE '%%:%%' AND UPPER(SUBSTR(n.sfld, 0, INSTR(n.sfld, ':'))) NOT IN (SELECT UPPER(title) FROM %s WHERE title NOT LIKE '%%:%%' AND tagNames LIKE '%%,%s,%%') " % (TABLES.EVERNOTE.NOTES, TAGS.TOC)
		}
	...
	if val not in _findAnknotes.note_ids or (not ANKNOTES.CACHE_SEARCHES and 'hierarchical' not in val):
		...
		log("  > Cached %s Note IDs: ".ljust(25) % val + "%-5s --> %3d results" % (tmr.str_long, len(_findAnknotes.note_ids[val])), tmr.label)
	else:
...
def anknotes_finder_query_wrap(self, preds=None, order=None, _old=None):
	...
	if "ank." in preds:
		sql = sql.replace("select c.id", "select distinct c.id").replace("from cards c", "from cards c, %s ank" % TABLES.EVERNOTE.NOTES)
...
def anknotes_profile_loaded():
	if not os.path.exists(os.path.dirname(FILES.USER.LAST_PROFILE_LOCATION)): os.makedirs(os.path.dirname(FILES.USER.LAST_PROFILE_LOCATION))
